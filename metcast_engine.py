...
import database_init_metcast as db_init

from database_init_metcast import Metcast, Location
...
class WeatherMaker:
    ...
    def __init__(self, name_location="Москва"):
        self.name_location = name_location
        self.week_metcast = list()
        self.db_metcast = DatabaseUpdater()

    def get_lacation(self):
        """
        Возвращает координаты объекта по его адресу
        :return: longitude (долгота), latitude (широта)
        """
        params = deepcopy(set_met.PARAMS_GEO)
        params["geocode"] = self.name_location
        response = requests.get("https://geocode-maps.yandex.ru/1.x", params=params)
        if response.status_code == 200:
            data = response.json()
            if data["response"]["GeoObjectCollection"]["featureMember"]:
                point_position = data["response"]["GeoObjectCollection"]["featureMember"][0]["GeoObject"]["Point"]["pos"]
                longitude, latitude = point_position.split()
            else:
                raise exc_met.YandexGeoLocationError("Could not determine current location")
        else:
            raise exc_met.YandexGeoResponseError("When requesting a geocoder, the response is not 200")
        return longitude, latitude

    def get_metcast(self):
        """
        Получение подробного прогзноза погоды на 7 дней вперед.
        """
        coincidence, longitude, latitude = self.db_metcast.get_location_base(current_location=self.name_location)
        if not coincidence:
            longitude, latitude = self.get_lacation()
    ...
    def date_selection_data(self, start_date_str, finish_date_str=None):
        """Выборка прогноза по дате в зависимсоти от ввода пользователя"""
        self.get_metcast()
...
class DatabaseUpdater:
    ...
    def append_result(self, mc, response, location_user):
        """Добавление запроса в итоговый рещультат"""
